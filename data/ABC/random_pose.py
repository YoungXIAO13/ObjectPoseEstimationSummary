...
parser.add_argument('--image_per_scene', type=int, default=100, help='images generated for each scene')
...
table_pose = np.load(join(root_dir, 'blender_render', 'table_poses.npz'))
...
outfile = join(args.dataset_dir, '{}.txt'.format(args.input))
frames = []
...
for scene_id in range(args.scenes):
    ...
    annotation_file = join(scene_out, 'scene_gt.json')
    scene_annot = {}
    ...
    render_machine = RenderMachine(model_files, scene_out, texture_dir=texture_dir, bg_dir=args.bg_dir, rad=3000)
    ...
    scene_df = pd.read_json(annotation_file, orient='index')
